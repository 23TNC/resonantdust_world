...
//!
//! Lifted verbatim from the previous iteration's `cards.rs` — schema-independent
//! and load-bearing for the `valid_at` model, so it's the first thing the new
//! shard inherits. Time is **milliseconds** throughout: `valid_at` rows pack u48
//! ms, durations are ms.
