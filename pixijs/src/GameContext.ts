...
export interface GameContext {
  ...
  /** The atlas pool — packs textures and serves the all-purpose `white` fill.
   *  Built at bootstrap once the renderer exists. */
  readonly textures: TextureManager;

  /** The DSL tile content runtime (def_id → colour), loaded at boot from the
   *  `content/` corpus through the `shared` wasm bundle. The world scene queries
   *  it per zone to expand packed tiles into renderable prims. */
