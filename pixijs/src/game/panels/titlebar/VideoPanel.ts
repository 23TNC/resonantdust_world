...
export class VideoPanel {
  ...
  constructor(app: Application, taskbar?: PanelTaskbar, uiEditMode?: UiEditMode) {
    this.app = app;
  ...
  /** Apply a render-scale to the renderer. `resolution` is the device-pixels
   *  per CSS-pixel factor; a resize re-bakes the backing buffers at the new
   *  scale (`autoDensity` keeps the CSS canvas size fixed). */
  private applyRenderScale(scale: number): void {
    if (!(scale > 0)) return;
    this.app.renderer.resolution = scale;
    this.app.renderer.resize(window.innerWidth, window.innerHeight, scale);
  }
