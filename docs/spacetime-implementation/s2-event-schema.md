...
[S1](s1-interpreter.md); converges at [S3](s3-worker.md). Design: [lifecycle.md](../spacetime-tables/lifecycle.md).
...
- Add the **lifecycle columns**: `status` (`enqueue|queueing|in_queue|running|complete|queue_failed`),
  `failed: bool`, `tic_state_change: u32` ([lifecycle.md](../spacetime-tables/lifecycle.md)).
...
- **`cold_removed`** — per-zone `removed: Vec<u16>` (`x:4|y:4|layer:4|type_id:4`): the removal
  ...
  (not GC) ([tables.md](../spacetime-tables/tables.md) §`cold_removed`).
...
- **`drop_timed_out()`** — **master-called, before `bump`**: `queue_failed` every still-
  ...
  causality barrier ([lifecycle.md](../spacetime-tables/lifecycle.md)).
...
- The `(source_shard, event_reference)` **provenance pair** stays as the **cross-shard
  ...
  cross-shard convergent writes — [lifecycle.md](../spacetime-tables/lifecycle.md)).
