...
- **[`intent/`](intent/)** — [`client.md`](intent/client.md): the shape + role, the host API
  (`src/api.rs`), login flow, configuration, the `headless` binary.
- `design/`, `current/`, `plan/` — lazy, per [CONVENTIONS](../../../CONVENTIONS.md); they come when
  this component is next worked.

Related: [`client/pixijs`](../pixijs/) (the browser display over this), and
[`docs/intent/sync.md`](../../../intent/sync.md) (the clock/interpolation model this client feeds).
