...
- Display: the **4 fresh** lights read their `shadow-hot` lane directly (zero-lag); the other **28** read
  their `shadow-warm` bit (a `bf_bit` **float-mod** extract, exact on rgba8 — the client's shaders are
  GLSL ES 1.00, see [`design/rendering-platform.md`](../design/rendering-platform.md)).
