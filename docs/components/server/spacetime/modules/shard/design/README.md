...
[divergences.md](../current/divergences.md).
...
- **[hot-cold.md](../intent/hot-cold.md)** — the hot/cold model, per-shape modules (tiles/pawns/things,
  each hot+cold), and why cold→hot mint is **absorbed into enqueue** (no `MINT`/`GET` verb);
  `pack` (settle) is an execute op.
- **[events.md](../intent/events.md)** — the event log carries `actions : Vec<u64>`; the shape of the
...
- **[lifecycle.md](../intent/lifecycle.md)** — **how an event resolves safely**: the event-shard/data-shard
...
- **[divergences.md](../current/divergences.md)** — the honest ledger: what the code does today vs
  this design, per table, with the fix.
- **[risks.md](../intent/risks.md)** — trade-offs and what could bite: build-a-slice-first, determinism,
  the drop-as-real-time-deadline trade, latency, and what's already been closed.
- **[../spacetime-implementation/](../plan/stages/README.md)** — the staged
...
4. ✅ **Nothing modifies a settled tic.** Work is right-shifted (data at N, execute at N+1,
   ...
   **structurally impossible** — no watermark; the master just **drops** any row that misses its
   ...
   (refcounted). The correctness core — see [lifecycle.md](../intent/lifecycle.md).
