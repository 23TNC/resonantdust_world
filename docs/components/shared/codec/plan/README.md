...
1. **Re-cut `object.rs` / `refs.rs` to the reference model** (2026-07-14 decision,
   ...
   `entity_reference = reserved:2 | realm_reference:8 | reference_id:6 | server_reference:16
   (realm:8 | server_id:8) | object_reference:32` (was `entity_type | entity_id | mint_server`).
