...
| `GRANT_MOODLET` | 11 | 2 | `obj:entity_reference` (**write**) · `moodlet_id` (imm, 1-based corpus id) — grant one STORED (timed) moodlet (needs-moodlets F2/F7). `grant_tic` = the composing tic; expiry is DERIVED (`grant_tic + duration` from the corpus), never stored; a re-grant refreshes the timer. Conditional (band) moodlets have no verb — they are derived, not granted. The drink action (the successor stream) queues this. |
