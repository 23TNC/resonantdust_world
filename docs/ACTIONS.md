...
| `SET_NEED` | 10 | 3 | `obj:entity_reference` (**write**) · `need_id` (imm, 1-based corpus id) · `satisfaction` (imm, quantised `0..=255`) — set one need's satisfaction on a pawn (needs-moodlets F7). The pawn module's `set_need` reducer splices the `NEED` payload entry itself (`set_tic` = the composing tic); the worker relays, adding nothing to its read set. The write operand serialises it with the pawn's movement writes. CLIENT-open like `MOVE_TO` while no ownership model exists (the npc mints thirst and the drills force-set through this door). |
| `GRANT_CONDITION` | 11 | 2 | `obj:entity_reference` (**write**) · `condition_id` (imm, 1-based corpus id) — grant one TIMED condition (needs-moodlets F2/F7). `grant_tic` = the composing tic; expiry is DERIVED (`grant_tic + duration` from the corpus), never stored; a re-grant refreshes the timer. DERIVED (band) conditions have no verb — they are computed, not granted. The drink action (the successor stream) queues this; until it exists the npc's `NPC_GRANT` drill is the only caller (`2026-08-04-conditions` F8). **Renamed from `GRANT_MOODLET`; the opcode and arity are unchanged.** |
