...
| action | value | arity | signature |
|---|---|---|---|
| `CREATE` | 3 | variable | `def:definition_reference` (imm) · `position:position_reference` (imm) · `count` (imm) · `payload×count` (imm) → **mints** a new entity — THE creation verb for any object (human-pawns F2). **WORKER-ONLY since spawn-authority F4** (it left `CLIENT_VERBS`; the `SPAWN_REQUEST` arm is its only composer — clients spawn by REQUESTING, the server validates and mints). The packed def's `type_id` **routes** the mint to its type's shard arm (`TYPE_PAWN` today; any other type is rejected by name in the worker, never silently pawned). `payload` is the minted entity's opcode-stream sidecar ([`TABLES.md` §payload](TABLES.md)) — e.g. a human's `PART(0, body def)`/`PART(1, head def)`, COMPOSED BY THE WORKER from the corpus + the request's variant nibbles (spawn-authority F3); empty for the wolf. The written target is the *minted* id, not an operand. The second variable-arity verb (with `INIT_ZONE`); the `count` word sits third in both. |
...
| `SPAWN_REQUEST` | 17 | 3 | `xy` (imm, `x:16 \| y:16` RAW tile coordinates — no client-side packing; the u16 lanes bound the in-world check) · `rot_def` (imm, `rotation:4 \| type:4 \| subtype:12 \| kind:12` — the def MINUS its variant nibble shifted down four; `def = (word & 0x0FFFFFFF) << 4 \| variant`; rotation seeds the initial FACING for pawns, 0–3, and flows into the SET `data` lane for cold types) · `variants` (imm, `v0:4 \| v1:4 \| … \| v7:4` — part i's variant at nibble i; the corpus `[[thing.part]]` declarations are the tail's CONTRACT and nonzero nibbles beyond the declared count REFUSE; the ≤4-parts law keeps this ONE word, so the verb frames corpus-free) — **THE SPAWN AUTHORITY DOOR** (spawn-authority F1/F2/F3/F6, the user's layout): the ONLY client-open spawn lane. Writes NOTHING itself — the worker VALIDATES (a registered def via the definitions mirror; in-world + PATHABLE for pawns; the named cell EMPTY on the thing layer for things; refuse never nudge) and ROUTES by the TYPE nibble: `TYPE_PAWN` → composes parts/traits/needs from ITS corpus and queues the worker-only `PROMOTE CREATE …` (the spawn ledger stays the one mint gate); `TYPE_BIOME_THING`/`TYPE_BIOME_TILE` → queues the validated `PROMOTE SET …` (the death-meat / build-wall lanes). A debug tool by design — likely removed once real spawn producers exist. |
| `RESTAMP_NEED` | 18 | 2 | `obj:server_reference` (**write**) · `need:definition_reference` (imm, the FULL u32 gameplay ref) — **THE CROSSING RE-STAMP** (survival F4/D1, WORKER-ONLY — never in `CLIENT_VERBS`): re-evaluate the pawn's named need's LAZY value AT PROCESSING and write it through `set_need`, feeding the same need-write sweep a `SET_NEED` feeds (death at corpus ≤ 0). Queued by the worker's crossing scheduler at the predicted floor-crossing tic (one pending slot per `(pawn, need)`, earliest wins, horizon-clamped); a stale prediction — the pawn ate — writes the HONEST current value and the sweep re-schedules. Never a literal predicted value: that is exactly the bug this verb exists to avoid. |
| `INV_ADD` | 15 | 2 | `obj:entity_reference` (**write**) · `item:definition_reference` (imm) — add an item to the pawn's `inventory` sub-table (inventory F3). The free-count law: `inventory` the NEED is FREE SLOTS, so an add spends one. |
| `INV_REMOVE` | 16 | 2 | `obj:entity_reference` (**write**) · `slot` (imm, 0-based) — remove the item in `slot`; same free-count law in reverse. |
| `ACTIVATE_TRAIT` | 19 | 2 | `obj:entity_reference` (**write**) · `tref:definition_reference` (imm, the ACTIVE trait's FULL reference) — **CLIENT-OPEN** (trait-rows-u32 F3): request activation. Writes nothing itself; the worker validates (the carrier BINDS the trait, the category is `*_active`, every `blocked_by` condition is inactive — sprint's cooldown gate) and queues the def's authored condition GRANTs. |
| `MOVE_CHORDS` | 20 | variable | `pawn:entity_reference` (imm — a display fan, no write set) · `entry_id` (imm, the `QUEUE_STATE` entry these chords belong to, so a client drops them when that entry cancels or completes) · `count` (imm, total payload WORDS — the **fifth** variable-arity verb, `count` third like the other four) · `chord payload×count` (imm) — **THE STAMPED ROUTE** (server-chords F1), **WORKER-ONLY**, always `PROMOTE_EVENT`-prefixed so it reaches the zone's subscribers. A chord is a straight segment crossing no impathable tile, spelled with BOTH endpoints as `position_reference`s and BOTH tics, so it is self-contained: a client interpolates between two stated points and derives no route, pace or position of its own. That is the design — the client's belief cannot drift from the server's because the client holds no belief, only two numbers and a clock. §Movement. |
| `CANCEL` | 21 | 1 | `obj:entity_reference` (**write** + **read**) — **THE FORMAL INTERRUPT** (server-chords F2). Bumps the pawn's trip-serial so every queued `MOVE_STEP` dies at the predicate it already checks, RESOLVES the pawn's position from the current tic, writes it under a `PROMOTE` prefix, and drops the intent queue (refanning an empty `QUEUE_STATE`). **CLIENT-OPEN** — a new player order composes `CANCEL` then the order in ONE program, so both land in one event and one group. Distinct from `CANCEL_INTENT` and NOT foldable into it: that verb's operands are both imm, so the pawn is no write target — it neither joins the pawn's write group nor serialises against its in-flight hops, and it writes nothing. A verb that PLACES a pawn must be a write, and a signature cannot be conditional on an operand's value. |
