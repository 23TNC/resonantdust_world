...
| `GRANT_CONDITION` | 11 | 2 | `obj:entity_reference` (**write**) · `row` (imm, the packed gameplay row `remaining_at_write:16 \| kind:12 \| variant:4` — stat-model F1/F3; the composer passes the condition's authored `duration` as `remaining_at_write`) — grant one TIMED condition (needs-moodlets F2/F7). `written_tic` = the composing tic; remaining-now and expiry are DERIVED at read, never stored; a re-grant refreshes the row. The reducer also RE-STAMPS the granted condition's affected need rows in the same transaction (stat-model F7 — the re-stamp law). DERIVED (band) conditions have no verb — they are computed, not granted. `EXECUTE_INTERACTION` is the real producer (drink → quenched); the npc's `NPC_GRANT` drill remains the forcing lane. |
