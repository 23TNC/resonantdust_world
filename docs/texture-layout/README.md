...
Status legend (same as [`docs/spacetime-tables/`](../spacetime-tables/README.md)): ✅ AGREED ·
...
- ✅ **`<type>/<subtype>/<kind>/<subkind>`** — the object-model taxonomy, the same axes as
  [`object_type_reference` + `object_kind_reference`](../references/object-reference.md)
...
| segment | is | notes |
|---------|-----|-------|
| `type` / `subtype` / `kind` / `subkind` | object-model taxonomy | [object-reference.md](../references/object-reference.md); `type` reserved → dirs use these exact words |
...
- ❓ **`linked/…` → taxonomy mapping.** Which `<type>/<subtype>` do walls/fences/rocks land
   ...
   ([object-reference.md](../references/object-reference.md)) — needs a type assignment (or a
