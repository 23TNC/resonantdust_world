...
| **cold mutation** | The `tile`/`thing` cold shards are built + live (below) — seeded, subscribed, rendered. *Mutating* them is not: the `UNPACK` → hot → `PACK` path and the `cold_removed` tombstone table ([`intent/world-storage/`](intent/world-storage/README.md)). Read-only today. |
...
**Built and live** — the static terrain layers ([`world-storage`](intent/world-storage/README.md)):
generated by worldgen, seeded on a zone's first subscribe, relayed by the edge, rendered by the
client. Position-addressed (one shard per type per zone), immutable in place — mutation is deferred
(the `UNPACK` → hot → `PACK` path + `cold_removed` tombstones, [Not shaped yet](#not-shaped-yet)).

## `tile` — the cold ground shard

Built + live ([`intent/world-storage/`](intent/world-storage/README.md)). One cold shard per type per
zone (server = the module), so the key drops the server and is a `u32`.
