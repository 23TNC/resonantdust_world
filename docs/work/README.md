...
| [es300-migration](es300-migration/README.md) | open | Migrate **every** client shader off Pixi high-shader (ES 1.00) onto raw **ES 3.00** — one standard, no ES1/ES3 split. Linchpin = a shared ES 3.00 program builder (replicates Pixi's camera plumbing once); then migrate screen → shadow → bake shaders, behaviour-preserving (float-mod → `uint` on the same unorm8 textures). Integer-texture / MRT wins are enabled follow-ons, not in scope. Follows `es300-hello`. |
