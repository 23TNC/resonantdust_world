...
| [light-data-texture](light-data-texture/README.md) | done | Feed per-light data to the shader through a **5×5 `RGBA32F` data texture** (one column per light) instead of uniforms/hardcoded colours — the mechanism dense many-lights needs. **L1–L4 verified 2026-07-20**: shadow colour read from the texture, rewritten every frame → per-light colour flicker proves the live JS→texture→shader path (float worked; ES 1.00 ruled out true `u32`). Extends `shadow-tiered`; L5 (position from texture) deferred to **shadows**. |
