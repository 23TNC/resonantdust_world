...
| [bitfield-rt](bitfield-rt/README.md) | open | De-risking experiment gating `shadows`: prove a packed **bitfield survives an RT round-trip** in isolation — a world-space integer RT, 256 rects in the (100,50) zone each set to one-hot `bit = i mod 24`, decoded to 24 colours via `/overlayRT`. Pass = the storage layer `shadow-cold` needs is proven. |
