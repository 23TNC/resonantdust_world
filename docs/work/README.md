...
| [es300-hello](es300-hello/README.md) | open | Prove we can hand-write and render a raw **GLSL ES 3.00** shader in the client (everything today is high-shader ES 1.00). A full-viewport quad drawn by a raw `#version 300 es` program using `uint`/bitwise syntax ES 1.00 can't compile — the foundation `caster-lut` C5's GPU cast (vertex-texture-fetch) and all future integer-texture / `texelFetch` / MRT work need. |
