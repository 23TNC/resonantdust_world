...
- [x] Instrument facing transitions (`console.debug`: old→new + source spec/chase/server)
...
- [x] Derive in-motion facing from the RENDERED per-tick delta `(rx−prev, ry−prev)` in
      ...
      a logged trip.
- [x] Add flip hysteresis: a facing holds ≥ a minimum interval (constant, ~150 ms)
...
- [x] Demote `walkGreedy`'s facing to the INITIAL aim only (the first frame of a spec,
      before any rendered delta exists). Acceptance: grep shows `p.facing` consumed
      nowhere else; typecheck.
- [x] Apply server facing ONLY at rest (no spec, chase gap ≈ 0) — and DO adopt it there:
...
- [x] The wolf soak drill: ≥ 3 trips watched at the fixture, plus a right-click human
      ...
      `completed.md`.
- [x] The records inherit: the prim `rotation` lane + `cast_type` follow the speculative
