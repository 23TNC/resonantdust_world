...
`cold_light_data` (1 px/light) · `cold_light_billboard_data` (the LUT, 4 entries/px, indices only) ·
`billboard_definition_data` (1 px/sprite variant, generic; `frame_page` + a spare `u32` for materials) ·
`cold_billboard_data` (2 entries/px: `position_anchor_reference` + `u8 z | u2 rotation | u22 reserved`). The
normalization — a caster's position lives only in `cold_billboard_data` — is the load-bearing choice: move a caster
...
## P1 · billboard_definition_data + atlas integration — 2026-07-21

New `ColdShadowData` module owning the shadow cast's GPU data textures. `billboard_definition_data` (`RGBA32UI`,
...
## P2 · cold_billboard_data (placed caster instances) + position codec — 2026-07-21

`cold_billboard_data` (`RGBA32UI`, 2 entries/px): each placed caster's `position_anchor_reference` + `z` +
...
## P3 · cold_light_data + cold_light_billboard_data (the LUT) — 2026-07-21

`cold_light_data` (`RGBA32UI`, 1 px/light): encoded position, colour+intensity (u8), radius+z in **units** +
`cast_shadows`, and the `(lut_index, lut_count)` run. `cold_light_billboard_data` (the LUT, 4 entries/px): each
`cast_shadows` light's in-range casters as a contiguous run of `(definition_index, billboard_data_index)`,
...
buffers + the old `depthFor` (depth now lives in `billboard_definition_data`). **Verified in-browser at
