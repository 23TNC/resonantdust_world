...
- [x] Scene reset (I1): wolves + bunnies + npc stopped, pawn module wiped,
      sims restarted, seed guard quiet, 0 pawns. Acceptance: SQL count 0;
      arcs green.
- [x] Row 1 — `NPC_COUNT=8`: mint (record actual N — I4), settle ≥1 min,
...
- [x] Row 2 — `NPC_COUNT=16`: same, ≥3-min soak. Acceptance: the row in
      completed.md.
- [x] Row 3 — `NPC_COUNT=24`: same, **≥10-min soak** (I2) + the zone spread
      of the population (I3). Acceptance: the row + the full lag series in
      completed.md.

## P2 — the verdict, and the truth

- [x] The verdict: per row IN-STEP / STABLE-BEHIND / GROWING from the lag
...
- [x] Restore the world (cast wiped — debug movers are deathless; wolves +
      bunnies back and re-minted); docs + memory truth pass; **the user's
      eyes close the stream**. Acceptance: docs-check green; arcs green.
      (Restored: wolf + warren re-minting, docs green; the torch-perf memory's
      superseded lag claim CORRECTED; B1 remains the user's.)
