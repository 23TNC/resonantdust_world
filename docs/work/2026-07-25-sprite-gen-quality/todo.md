...
- [x] Rebuild the training set and report the before/after distribution. Acceptance: `fill` standard deviation across the corpus drops below 0.02, stated in `completed.md`.
- [x] Spot-check 6 rebuilt sprites against their sources for artefacts. Acceptance: 6 side-by-side pairs saved; no halo, clipping, or aspect distortion found, or the defect is logged in `issues.md`.
