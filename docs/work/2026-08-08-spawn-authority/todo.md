...
- [x] ACTIONS.md: the `SPAWN_REQUEST def position variants` verb (request → the
...
- [x] Codec: `SPAWN_REQUEST` (3 × Imm — [x:16|y:16] [rot:4|type:4|sub:12|kind:12]
...
- [x] Edge: SPAWN_REQUEST joins CLIENT_VERBS, CREATE LEAVES it (F4); the FULL
...
- [x] Worker: the SPAWN_REQUEST arm — reconstruct def + rotation from word 2;
...
- [x] Worker: `mint_parts` — PART entries composed server-side per the kind's
...
- [x] Worker: the TYPE router (F6) — TYPE_BIOME_THING → the cold-overlay SET
...
- [x] webgl `/spawn`: DELETE the part-packing; send `SPAWN_REQUEST def pos
...
- [x] npc: wolves + bunnies mint via SPAWN_REQUEST (keep the pathable pick as
