...
- [x] **Define the item rule**: an ITEM is a line matching `^\s*[-*]\s+\[[ xX]\]` in `todo.md` /
...
- [x] **Normalize the corpus** — convert the 14 non-checkbox streams' `todo.md`/`remaining.md` items to
...
- [x] **`docs-check` invariant `work-items`** — a non-DONE `##` section in a `todo.md` that contains
...
- [x] **Retire the heuristics** in `work_check.py`: `DONE_HEADER` scanning for item state, the
...
- [x] **Progress signal from box counts** — the guard's progress signature uses `(open, done)` box
...
- [x] **Decision log** — append every Stop verdict to `.git/rd-work/decisions.jsonl`:
...
- [x] **`rd work-check --doctor`** — per-stream table of what the parser sees (status, items open/done,
...
- [x] **Wire doctor into `docs-check` as a WARNING** so an unreadable stream announces itself in the
...
- [x] **`rd work arm <stream>` / `disarm` / `status`** — persistent per-session arm state in
...
- [x] **Hook nudges only when armed** — an unarmed session ends its turn silently, always. The
...
- [x] **Auto-disarm** on the three real exits: all items ticked, a blocker recorded, or `.stop-reason`
      written. Acceptance: recording a blocker while armed disarms and the next stop is silent.
- [x] **Escalate on stall** — when the guard releases after `MAX_NUDGES` no-progress stops, auto-write
...
- [x] **`/rd-execute` skill** (`.claude/skills/rd-execute/`) — the user-facing "go": pick/confirm the
...
- [x] **`rd work brief <stream>`** — assemble the compact resume payload: current phase, next 3
...
- [x] **Hook emits the brief** instead of the bare item list, keeping the four legitimate exits.
      Acceptance: a nudge shows the brief; length stays bounded (cap + "…N more").

## P4 — Planning assist (the gap hooks can't reach)

- [x] **Granularity warning** in `docs-check` (advisory): flag an item over ~250 chars, or with ≥2
...
- [x] **`/rd-plan` skill** (`.claude/skills/rd-plan/`) — the planning counterpart to `/rd-execute`.
...
- [x] **Adopt "`[x]` IS the move"** — items stay in the plan file for the life of the stream;
...
- [x] **Corpus harness** — `bin/lib/work_check_selftest.py`: asserts the classifier against every
...
- [x] **Full hook matrix re-run** after every phase lands: blocked→allow, open work→block, nudge
