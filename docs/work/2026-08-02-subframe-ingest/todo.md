...
- [x] Add `&thing.subframe.x/y/w/h` as FRACTIONS to `shared/dsl` loader + `VisualParts`. Acceptance: a `.rd` authoring them round-trips through the loader's own test corpus, defaults being the full frame `(0,0,1,1)`.
