...
- [x] Add `offset.z` to the pawn DSL and retire `head.offset.y` ([F5](forks.md#f5)). Acceptance: the corpus authors a height; no y-fudge remains on the head slot.
...
- [x] Capture the A/B set at zoom 3. Acceptance: ground prim unchanged, head aligned, one shadow; each beside its before-image.
