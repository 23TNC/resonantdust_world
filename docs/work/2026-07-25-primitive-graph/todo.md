...
- [ ] Set `child = 1` + `parent_id` when a prim is authored under another; verify a 2-level chain resolves.
- [ ] Sum a non-zero child offset in `resolveCarried`; verify against a hand-computed position.
...
- [ ] Clamp `z + z_offset` to u8 and note the ceiling ([I9](issues.md#i9)).
