...
- [ ] Build the atlas quadtree packer with the 4 maps of a definition in one 2×2 quadrant block. Acceptance: `fullframe.span == 2 × frame.span`; a packed definition's 4 maps resolve at the offsets the design derives, verified by sampling.
...
- [ ] Measure cost per light with no shadows. Acceptance: ms at N = 1, 4, 8, 16 lights — the first real bound on the per-pixel pass, taken BEFORE anything is built on top of it.

## P4 — The shadow gather: per unit, ping-ponged

- [ ] Allocate the shadow buffer at **3 px per UNIT**, ping-ponged, cleared to 0 ([F4](forks.md#f4)). Acceptance: sized ~6 MB not ~24 KB, and the first frame reads zeros rather than garbage.
...
- [ ] Store the ground caster and the 8 `(caster, receiver)` pairs, with the corrected `l >= 4` slot split. Acceptance: all 8 lights address distinct slots; light 4 lands in px 2 slot 0, which the design's `l > 4` dropped.
