...
- [x] `BuildPanel` (`game/panels/build/`): DomPanel with category sub-tabs (wall), icons
...
- [x] Placement mode: icon click enters; right-click exits; left-drag forms the rect
...
- [x] Blueprint preview: an overlay drawing the drag rect's PERIMETER with the blueprint
...
- [x] Docs first: `ACTIONS.md` + `VARIABLES.md` rows for action 9 `BUILD_WALL` (start
...
- [x] codec + edge + core: action 9 in `shared/codec`, edge CLIENT_VERBS allowlist,
...
- [x] Worker: handle BUILD_WALL — expand the rect perimeter, write the wall kind into the
...
- [x] End-to-end: enter mode → drag a rect → release → walls appear with correct variants
