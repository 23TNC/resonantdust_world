...
impl MoverTrack {
    ...
    pub fn observe_intent(&mut self, entity: u32, dest: (f64, f64), event_tic: u16) -> bool {
    ...
    /// Set a pawn's derived pace (tics per tile). Until this is known the pawn is held still —
    /// see [`Mover::pace`].
    pub fn set_pace(&mut self, entity: u32, pace: f64) {
    ...
    pub fn point_at(
        ...
        let (Some(dest), Some(pace)) = (m.dest, m.pace) else { return Some(m.point) };
        Some(resonantdust_content::move_eval::position_at(
            m.point, dest, m.tic, now, pace, pathable,
        ))
    }
}
...
mod tests {
    ...
    fn track_with_pawn() -> MoverTrack {
        ...
        assert!(t.observe_state(1, 0, 0, (10.0, 10.0), 0, 100));
        t.set_pace(1, 24.0);
        t
    }
    ...
    fn a_walking_pawn_moves_between_anchors() {
        ...
        assert!(t.observe_intent(1, (30.0, 10.0), 102));
        let at_start = t.point_at(1, 100, &open).unwrap();
        let mid = t.point_at(1, 112, &open).unwrap();
    ...
    fn an_unpaced_pawn_holds_position() {
        let mut t = MoverTrack::new();
        t.observe_state(1, 0, 0, (10.0, 10.0), 0, 100);
        t.observe_intent(1, (30.0, 10.0), 102);
        assert_eq!(t.point_at(1, 400, &open), Some((10.0, 10.0)));
    }
    ...
    fn an_older_row_is_rejected() {
        let mut t = track_with_pawn();
        assert!(t.observe_state(1, 0, 0, (12.0, 10.0), 0, 132));
        assert!(!t.observe_state(1, 0, 0, (10.0, 10.0), 0, 100));
    ...
    fn a_replayed_intent_is_rejected() {
        let mut t = track_with_pawn();
        assert!(!t.observe_intent(1, (30.0, 10.0), 100u16.wrapping_sub(500)));
        assert!(!t.get(1).unwrap().walking());
        // ...while an intent inside the barrier arms normally.
        assert!(t.observe_intent(1, (30.0, 10.0), 104));
        assert!(t.get(1).unwrap().walking());
        // ...and a serially older one no longer supersedes it.
        assert!(!t.observe_intent(1, (5.0, 10.0), 103));
    }
    ...
    fn arriving_clears_the_walk() {
        let mut t = track_with_pawn();
        t.observe_intent(1, (12.0, 10.0), 102);
        assert!(t.get(1).unwrap().walking());
        t.observe_state(1, 0, 0, (12.0, 10.0), 1, 150);
        assert!(!t.get(1).unwrap().walking());
        assert_eq!(t.point_at(1, 900, &open), Some((12.0, 10.0)));
    }
    ...
    fn a_closed_zone_drops_its_pawns() {
        let mut t = MoverTrack::new();
        t.observe_state(1, 7, 0, (10.0, 10.0), 0, 100);
        t.observe_state(2, 8, 0, (20.0, 20.0), 0, 100);
        t.forget_zone(7);
