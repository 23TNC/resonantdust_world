...
export class RecordSync {
  ...
  private defFields(resolver: TextureResolver, stem: string, cell: number, boxSpanHint: number):
    ...
    // one-resolution F4: COORDS from the ALBEDO quadrant, SOURCE from the DATA twin. The
    // silhouette consumers (`uSurfaceAtlas`) reach the surface by adding `+frameUnits` to the
    // def's frame — so frameX/frameY must stay the albedo quadrant's — but the surface pixels
    // live on the DATA page (the graphics twin's surface quadrant is EMPTY: registering the
    // albedo source bound graphics and every silhouetteHit missed — the world lost its shadows).
    const frame = resolver.resolve(stem, "albedo", cell)?.frame;
    if (!frame) return null;
    const surfSrc = resolver.resolve(stem, "surface", cell)?.frame?.source ?? frame.source;
    let page = this.pages.indexOf(surfSrc);
    if (page < 0) { this.pages.push(surfSrc); page = this.pages.length - 1; }
