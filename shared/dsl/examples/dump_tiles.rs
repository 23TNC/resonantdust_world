...
//!   cargo run -p resonantdust-dsl --example dump_tiles -- <content_root>

use std::path::PathBuf;

fn main() {
  let root: PathBuf = std::env::args().nth(1).unwrap_or_else(|| "content".into()).into();
  let sources = resonantdust_dsl::content::read_content_dir(&root)
    .unwrap_or_else(|e| panic!("read {}: {e}", root.display()));
  println!("read {} source files from {}", sources.len(), root.display());
  let bundle = resonantdust_dsl::load(&sources).expect("load content");
