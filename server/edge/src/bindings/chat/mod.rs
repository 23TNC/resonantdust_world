...
fn args_bsatn(&self) -> Result<Vec<u8>, __sats::bsatn::EncodeError> {
        match self {
                ...
                sender_player_id: sender_player_id.clone(),
...
#[doc(hidden)]
pub struct DbUpdate {
        chat_messages: __sdk::TableUpdate<ChatMessage>,
}
...
impl TryFrom<__ws::v2::TransactionUpdate> for DbUpdate {
    ...
    fn try_from(raw: __ws::v2::TransactionUpdate) -> Result<Self, Self::Error> {
        ...
        "chat_messages" => db_update.chat_messages.append(chat_messages_table::parse_table_update(table_update)?),
...
for table_rows in raw.tables {
            match &table_rows.table[..] {
                                "chat_messages" => db_update.chat_messages.append(__sdk::parse_row_list_as_inserts(table_rows.rows)?),
...
for table_rows in raw.tables {
            match &table_rows.table[..] {
                                "chat_messages" => db_update.chat_messages.append(__sdk::parse_row_list_as_deletes(table_rows.rows)?),
                unknown => { return Err(__sdk::InternalError::unknown_name("table", unknown, "QueryRows").into()); }
}}        Ok(db_update)
}
}
...
pub struct AppliedDiff<'r> {
        chat_messages: __sdk::TableAppliedDiff<'r, ChatMessage>,
...
impl<'r> __sdk::AppliedDiff<'r> for AppliedDiff<'r> {
    fn invoke_row_callbacks(&self, event: &EventContext, callbacks: &mut __sdk::DbCallbacks<RemoteModule>) {
                callbacks.invoke_table_row_callbacks::<ChatMessage>("chat_messages", &self.chat_messages, event);
}
}
...
fn register_tables(client_cache: &mut __sdk::ClientCache<Self>) {
                chat_messages_table::register_table(client_cache);
}
