...
pub enum ServerMsg {
    ...
    /// A pawn THIS session's player minted (npc-host I11/F4 — the ownership re-attach lane):
    /// relayed from the pawn spawn ledger's issuer attribution at login and on new mints. A
    /// module brain commands ONLY refs it received here.
