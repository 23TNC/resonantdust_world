...
pub mod experiment_object_type;
pub mod set_experiment_position_reducer;
pub mod experiment_objects_table;

pub use experiment_object_type::ExperimentObject;
pub use experiment_objects_table::*;
pub use set_experiment_position_reducer::set_experiment_position;
...
impl __sdk::Reducer for Reducer {
    fn reducer_name(&self) -> &'static str {
        match self {
                        Reducer::SetExperimentPosition { .. } => "set_experiment_position",
            _ => unreachable!(),
}
}
...
fn args_bsatn(&self) -> Result<Vec<u8>, __sats::bsatn::EncodeError> {
        match self {
                        Reducer::SetExperimentPosition{
                object_id,
                x,
                y,
}             => __sats::bsatn::to_vec(&set_experiment_position_reducer::SetExperimentPositionArgs {
...
#[doc(hidden)]
pub struct DbUpdate {
        experiment_objects: __sdk::TableUpdate<ExperimentObject>,
}
...
impl TryFrom<__ws::v2::TransactionUpdate> for DbUpdate {
    ...
    fn try_from(raw: __ws::v2::TransactionUpdate) -> Result<Self, Self::Error> {
        ...
        "experiment_objects" => db_update.experiment_objects.append(experiment_objects_table::parse_table_update(table_update)?),
...
impl __sdk::DbUpdate for DbUpdate {
    fn apply_to_client_cache(&self, cache: &mut __sdk::ClientCache<RemoteModule>) -> AppliedDiff<'_> {
                ...
                diff.experiment_objects = cache.apply_diff_to_table::<ExperimentObject>("experiment_objects", &self.experiment_objects).with_updates_by_pk(|row| &row.object_id);
...
for table_rows in raw.tables {
            match &table_rows.table[..] {
                                "experiment_objects" => db_update.experiment_objects.append(__sdk::parse_row_list_as_inserts(table_rows.rows)?),
...
for table_rows in raw.tables {
            match &table_rows.table[..] {
                                "experiment_objects" => db_update.experiment_objects.append(__sdk::parse_row_list_as_deletes(table_rows.rows)?),
                unknown => { return Err(__sdk::InternalError::unknown_name("table", unknown, "QueryRows").into()); }
}}        Ok(db_update)
}
}
...
pub struct AppliedDiff<'r> {
        experiment_objects: __sdk::TableAppliedDiff<'r, ExperimentObject>,
...
impl<'r> __sdk::AppliedDiff<'r> for AppliedDiff<'r> {
    fn invoke_row_callbacks(&self, event: &EventContext, callbacks: &mut __sdk::DbCallbacks<RemoteModule>) {
                callbacks.invoke_table_row_callbacks::<ExperimentObject>("experiment_objects", &self.experiment_objects, event);
}
}
...
fn register_tables(client_cache: &mut __sdk::ClientCache<Self>) {
                experiment_objects_table::register_table(client_cache);
}
