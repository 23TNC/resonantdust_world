...
pub mod can_exist_type;
pub mod cold_zone_type;
...
fn args_bsatn(&self) -> Result<Vec<u8>, __sats::bsatn::EncodeError> {
        match self {
                ...
                rotations,
                ids,
}             => __sats::bsatn::to_vec(&apply_hot_reducer::ApplyHotArgs {
                now_ms: now_ms.clone(),
                layer: layer.clone(),
            ...
            Reducer::SeedColdZone{
                ...
                tiles,
                things,
}             => __sats::bsatn::to_vec(&seed_cold_zone_reducer::SeedColdZoneArgs {
                now_ms: now_ms.clone(),
                zone_id: zone_id.clone(),
                tiles: tiles.clone(),
                things: things.clone(),
            ...
            Reducer::SetHot{
                ...
                rotation,
                id,
}             => __sats::bsatn::to_vec(&set_hot_reducer::SetHotArgs {
                now_ms: now_ms.clone(),
                layer: layer.clone(),
                zone_id: zone_id.clone(),
...
#[doc(hidden)]
pub struct DbUpdate {
        can_exist: __sdk::TableUpdate<CanExist>,
...
impl TryFrom<__ws::v2::TransactionUpdate> for DbUpdate {
    ...
    fn try_from(raw: __ws::v2::TransactionUpdate) -> Result<Self, Self::Error> {
        ...
        "can_exist" => db_update.can_exist.append(can_exist_table::parse_table_update(table_update)?),
...
impl __sdk::DbUpdate for DbUpdate {
    fn apply_to_client_cache(&self, cache: &mut __sdk::ClientCache<RemoteModule>) -> AppliedDiff<'_> {
                ...
                diff.can_exist = cache.apply_diff_to_table::<CanExist>("can_exist", &self.can_exist).with_updates_by_pk(|row| &row.zone_id);
...
for table_rows in raw.tables {
            match &table_rows.table[..] {
                                "can_exist" => db_update.can_exist.append(__sdk::parse_row_list_as_inserts(table_rows.rows)?),
...
for table_rows in raw.tables {
            match &table_rows.table[..] {
                                "can_exist" => db_update.can_exist.append(__sdk::parse_row_list_as_deletes(table_rows.rows)?),
...
pub struct AppliedDiff<'r> {
        can_exist: __sdk::TableAppliedDiff<'r, CanExist>,
...
impl<'r> __sdk::AppliedDiff<'r> for AppliedDiff<'r> {
    fn invoke_row_callbacks(&self, event: &EventContext, callbacks: &mut __sdk::DbCallbacks<RemoteModule>) {
                callbacks.invoke_table_row_callbacks::<CanExist>("can_exist", &self.can_exist, event);
...
fn register_tables(client_cache: &mut __sdk::ClientCache<Self>) {
                can_exist_table::register_table(client_cache);
